...
class TestCreateCharacter(TestCase):
    ...
    def test_generate_character_syllable_0(self, mock_select_class):
        self.assertEqual(dungeonsanddragons.generate_character(0), None)
    ...
    def test_generate_character_syllable_less_or_equal_0_expected_output(self, mock_select_class, mock_stdout):
    ...
    def test_generate_character_strength_within_boundaries(self, mock_select_class):
        character = dungeonsanddragons.generate_character(2)
        self.assertTrue(3 <= character["Strength"] <= 18)

    @patch('dungeonsanddragons.select_class', return_value="monk")
    def test_generate_character_dexterity_within_boundaries(self, mock_select_class):
        character = dungeonsanddragons.generate_character(2)
        self.assertTrue(3 <= character["Dexterity"] <= 18)

    @patch('dungeonsanddragons.select_class', return_value="warlock")
    def test_generate_character_constitution_within_boundaries(self, mock_select_class):
        character = dungeonsanddragons.generate_character(2)
        self.assertTrue(3 <= character["Constitution"] <= 18)

    @patch('dungeonsanddragons.select_class', return_value="fighter")
    def test_generate_character_intelligence_within_boundaries(self, mock_select_class):
        character = dungeonsanddragons.generate_character(2)
        self.assertTrue(3 <= character["Intelligence"] <= 18)

    @patch('dungeonsanddragons.select_class', return_value="wizard")
    def test_generate_character_wisdom_within_boundaries(self, mock_select_class):
        character = dungeonsanddragons.generate_character(2)
        self.assertTrue(3 <= character["Wisdom"] <= 18)

    @patch('dungeonsanddragons.select_class', return_value="cleric")
    def test_generate_character_charisma_within_boundaries(self, mock_select_class):
        character = dungeonsanddragons.generate_character(2)
